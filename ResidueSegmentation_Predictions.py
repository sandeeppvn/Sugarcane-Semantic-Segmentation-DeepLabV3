#RUN this file after ResidueSegmentation_Training.py to generate predictions as a gif
import numpy as np
import argparse
...
# if not os.path.exists(os.path.join('./outputs/GIF/', exp_name)):
#     os.makedirs(os.path.join('./outputs/GIF/', exp_name))
# else:
#     shutil.rmtree(os.path.join('./outputs/GIF/', exp_name))
#     os.makedirs(os.path.join('./outputs/GIF/', exp_name))
...
# Create dictionary of image name to list of predictions
prediction_dict = {}


# For each model saved, generate prediciton
for idx,model_path in enumerate(sorted(glob(os.path.join('./saved_models', exp_name, '*.pth')))):
    print('\nGenerating mask prediction for model: {}'.format(model_path))
    ...
    writer.add_scalar('test/IoU', results['iou_score'], idx)
    writer.add_scalar('test/loss', results['dice_loss'], idx)
    '''
    # Generate mask prediction
    print()
    print('Generating mask prediction for model: {}'.format(model_path))
    with torch.no_grad():
        for idx,(image,mask) in enumerate(tqdm(test_dataloader)):
            ...
            mask_name = (index_to_mask_name_dict[idx].split('/')[-1]).replace('PNG','GIF')

            if mask_name not in prediction_dict:
                prediction_dict[mask_name] = [res]
            else:
                prediction_dict[mask_name].append(res)
    
print()
# Save predictions as a gif
for key in prediction_dict:
    print('Saving gif for model: {}'.format(key))
    save_path = os.path.join('./outputs/GIF/', exp_name, '{}'.format(key))
    images = prediction_dict[key]
    images[0].save(
        save_path,
        save_all=True, 
